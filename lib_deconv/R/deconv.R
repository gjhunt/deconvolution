...
#' @param gamma Expression sensitivity parameter. Single positive numeric. If not supplied then \code{gamma} will be determined by the function \code{\link{est_gamma}}.
...
#' @param seed Seed for randomization used when adaptively choosing \code{n_choose} or \code{gamma}. 
...
#' @export
deconv <- function(Y, pure_samples, n_choose = NULL, full_markers = NULL, gamma = NULL, 
    n_test = 500, n_test_reps = 2, gamma_test_reps = 2, seed = NULL) {
    ...
    estimate_gamma_flag <- is.null(gamma)
    if (estimate_gamma_flag) {
        gamma <- 0.7
    }
    
    if (estimate_gamma_flag | is.null(n_choose)) {
    ...
    if (estimate_gamma_flag) {
        gamma <- est_gamma(td, pure_samples, full_markers, gamma, n_choose, gamma_test_reps)
    }
