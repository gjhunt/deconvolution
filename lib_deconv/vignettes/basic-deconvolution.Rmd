...
Given the data and the pure samples we can run our deconvolution algorithm. First let's run it blindly just supplying the expression matrix and the pure sample list. 
```{r,cache=FALSE}
library('deconv')
dc <- deconv(Y, pure_samples)
phats <- dc$estimates
head(phats)
```
...
```{r}
marks = find_markers(Y,pure_samples)
dc <- deconv(Y, pure_samples, n_choose = c(100,101,102), gamma=.7, full_markers = marks)
phats <- dc$estimates
head(phats)
...
We can also let deconv adaptively choose quantities. 
```{r}
td = gen_test_data(Y,pure_samples)
n_choose = est_n(td,pure_samples,marks,.7,rep(50,3),n_reps=1,err.plot=TRUE)
gamma = est_gamma(td,pure_samples,marks,.7,n_choose,n_reps=1,err.plot=TRUE)
```


Doing these in advance often allows us to play with parameters without having to recompute them each time. 
```{r}
dc <- deconv(Y, pure_samples,n_choose,marks,gamma)
phats <- dc$estimates
plot(rat.truth,phats,xlab="Truth",ylab="Estimates",xlim=c(0,1),ylim=c(0,1))
abline(coef=c(0,1))
dc2 <- deconv(Y, pure_samples,n_choose,marks,.7)
phats2 <- dc2$estimates
points(rat.truth,phats2,col='blue')
dc3 <- deconv(Y, pure_samples,200,marks,gamma)
